...
def ask_ai(prompt):
    ...
    data = {
        ...
        "messages": [
            {"role": "system", "content": "you are a chaotic gen z discord bot."},
...
async def ship(interaction: discord.Interaction, user1: discord.Member, user2: discord.Member):
    await interaction.response.send_message(
        f"{user1.mention} + {user2.mention} = {random.randint(0,100)}%"
    )
...
async def guess_number(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("🎮 Guess 1-100 | 10 attempts | 1 min")
    ...
    if interaction.channel.id in game_state:
        ...
        embed = discord.Embed(
            title="Time's up",
            description=f"Answer was **{answer}**",
            color=0x1B2B5B
        )
...
async def guess_member(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("👤 Guess the member")

    await asyncio.sleep(60)

    if interaction.channel.id in member_game:

        member = interaction.guild.get_member(member_game[interaction.channel.id])
...
async def on_message(message):
        ...
        if message.content.isdigit():
            ...
            if game["attempts"] <= 0:
                ...
                embed = discord.Embed(
                    ...
                    description=f"Answer was {game['answer']}",
                    color=0x1B2B5B
                )
            ...
            await message.channel.send("higher" if guess < game["answer"] else "lower")
            return
    ...
    if message.channel.id in member_game:
        ...
        member = message.guild.get_member(member_game[message.channel.id])

        if member and (
            guess in member.display_name.lower() or guess in member.name.lower()
        ):
    ...
    if message.author.id in allowed and msg.startswith("spam "):
    ...
    for k, v in responses.items():
        if k in msg:
            await message.channel.send(embed=discord.Embed(description=v, color=0x1B2B5B))
            return
    ...
    # -------------------------
    if ai_enabled:
        await message.channel.send(ask_ai(message.content))

    # -------------------------
    # 🔥 GLAZE SYSTEM (NEW)
    # -------------------------
    if msg.startswith("glaze "):
        ...
        parts = msg.split()
        style = parts[2] if len(parts) > 2 else "sigma"

        prompt = f"write a short discord glaze about {target.display_name} in {style} style"
